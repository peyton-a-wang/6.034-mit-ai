...
def extensions(graph, path):
    ...
    for node in neighbors:
        ext = path.copy()
        ext.append(node)
        if not has_loops(ext):
            extensions.append(ext)

    return break_ties(extensions)
...
def basic_dfs(graph, startNode, goalNode):
    ...
    while len(paths) != 0:
        ...
        if current_node == goalNode:
            ...
            extended_paths = []
            for ext_path in extensions(graph, [current_node]):
                new_ext = current_path.copy()
                child = ext_path[-1]
                new_ext.append(child)
                if not has_loops(new_ext):
                    extended_paths.append(new_ext)
            
            paths = extended_paths + paths           
...
def path_len_sorting(graph, goalNode, paths):
    return sorted(paths, key=lambda path: path_length(graph, path))

def heuristic_path_len_sorting(graph, goalNode, paths):
...
generic_best_first = [heuristic_sorting, False, do_nothing_fn, False]

generic_branch_and_bound = [do_nothing_fn, False, path_len_sorting, False]

generic_branch_and_bound_with_heuristic = [path_len_sorting, False, heuristic_path_len_sorting, False]

generic_branch_and_bound_with_extended_set = [do_nothing_fn, False, path_len_sorting, True]

generic_a_star = [do_nothing_fn, False, heuristic_path_len_sorting, True]
...
def is_admissible(graph, goalNode):
    ...
    for node in graph.nodes:
        shortest_path = path_length(graph, generic_search(*generic_branch_and_bound)(graph, node, goalNode))
        if graph.get_heuristic_value(node, goalNode) > shortest_path:
            return False
    return True
...
WHAT_I_FOUND_INTERESTING = """How small changes can affect the entire search algorithm."""
WHAT_I_FOUND_BORING = "Trying to figure out how generic_search worked."
SUGGESTIONS = "N/A"
